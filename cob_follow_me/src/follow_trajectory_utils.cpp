...
geometry_msgs::Pose FollowTrajectoryUtils::getPose(const std::string& target_frame, const std::string& source_frame)
{
...
tf::StampedTransform FollowTrajectoryUtils::getStampedTransform(const std::string& target_frame, const std::string& source_frame)
{
    tf::StampedTransform stamped_transform;
    ...
    do
    {
        ...
        {
            ROS_ERROR("FollowTrajectoryUtils::getStampedTransform: \n%s", ex.what());
...
void FollowTrajectoryUtils::transformPose(const std::string source_frame, const std::string target_frame, const geometry_msgs::Pose pose_in, geometry_msgs::Pose& pose_out)
{
    bool transform = false;
    ...
    do
    {
        ...
        {
            ROS_ERROR("FollowTrajectoryUtilsUtils::transformPose: \n%s", ex.what());
            ros::Duration(0.1).sleep();
        }
    } while (!transform && ros::ok());
}
...
bool FollowTrajectoryUtils::inEpsilonArea(const tf::StampedTransform& stamped_transform, const double epsilon)
{
...
void FollowTrajectoryUtils::poseToRPY(const geometry_msgs::Pose& pose, double& roll, double& pitch, double& yaw)
{
    tf::Quaternion q;
...
void FollowTrajectoryUtils::previewPath(const geometry_msgs::PoseArray pose_array)
{
...
double FollowTrajectoryUtils::roundUpToMultiplier(const double numberToRound, const double multiplier)
{
